...
export default function InvestmentTerminal() {
  ...
  // -- Permissions --
  // Market is open if the DB cluster says 'bidding' OR 'bidding_open' is true
  const isMarketOpen = cluster?.current_stage === 'bidding' || (cluster as any)?.bidding_open === true;
  // User can invest if they are Team Lead and their team hasn't finalized yet
  const isTeamLead = profile?.role === 'team_lead' || profile?.role === 'super_admin';
  const isFinalized = myTeam?.is_finalized ?? false;
  const canInvest = isMarketOpen && isTeamLead && !isFinalized;
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      setUser({ id: data.profile.id, email: data.profile.email } as User);
      ...
      // Load existing investments as drafts
  ...
  const handleCommit = async () => {
    if (!canInvest || isNegative || currentDraftTotal === 0) return;
  ...
  return (
    <main className="min-h-screen bg-[#0A0A0A] px-4 py-6 pb-32 relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-md mx-auto">
          ...
          {targetTeams.length === 0 ? (
            ...
            targetTeams.map((team) => {
              ...
              return (
                <div
                  key={id}
                  className={`bg-[#121212] border rounded-xl overflow-hidden transition-all duration-300 ${active ? 'border-[#FFD700] shadow-[0_0_20px_rgba(255,215,0,0.05)]' : 'border-[#262626]'}`}
                >
                  <div className="p-4">
                    {/* Card Header */}
                    ...
                    {/* Controls */}
                    <div className={`bg-[#0A0A0A] rounded-lg border flex items-center h-14 relative overflow-hidden transition-colors ${active ? 'border-[#FFD700]/30' : 'border-[#222]'}`}>

                      {/* Decrement */}
                      <button
                        onClick={() => adjustAmount(id, -STEP_AMOUNT)}
                        disabled={!canInvest}
                        className="w-14 h-full flex items-center justify-center text-gray-500 hover:text-white hover:bg-[#1A1A1A] transition-colors disabled:opacity-30 border-r border-[#222] z-10"
                      >
                      ...
                      <button
                        onClick={() => adjustAmount(id, STEP_AMOUNT)}
                        disabled={!canInvest}
                        className="w-14 h-full flex items-center justify-center text-[#FFD700] hover:bg-[#1A1A1A] transition-colors disabled:opacity-30 border-l border-[#222] z-10"
                      >
                        <span className="text-2xl font-light">+</span>
                      </button>
        ...
        {/* 5. Action Bar (Fixed Floating above Nav) */}
        {!isFinalized && canInvest && currentDraftTotal > 0 && (
